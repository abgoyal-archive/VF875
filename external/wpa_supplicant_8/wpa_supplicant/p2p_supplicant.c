 ...
 */
#ifdef ANDROID_P2P
#define P2P_MAX_CLIENT_IDLE 60
#define P2P_MAX_OWNER_IDLE 120
#else
...
static void wpas_p2p_group_delete(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->p2p_group_interface != NOT_P2P_GROUP_INTERFACE) {
		...
		wpa_printf(MSG_INFO, "P2P: Remove group interface %s",
			wpa_s->ifname);
		global = wpa_s->global;
	...
	wpa_printf(MSG_INFO, "P2P: Remove temporary group network");
	if (ssid && (ssid->p2p_group ||
		...
		wpa_printf(MSG_INFO, "P2P: Temporary group network not "
			   "found");
	}
	eloop_cancel_timeout(wpas_p2p_group_idle_timeout, wpa_s, NULL);
...
void wpas_start_tethering(struct wpa_supplicant *wpa_s,
			      struct p2p_go_neg_results *params)
{
	struct wpa_ssid *ssid;
	...
	ssid->auth_alg = WPA_AUTH_ALG_OPEN;
	ssid->key_mgmt = WPA_KEY_MGMT_PSK;
	ssid->proto = wpa_s->ssid_t.proto;
	ssid->pairwise_cipher = wpa_s->ssid_t.pairwise_cipher;
	ssid->passphrase = os_strdup(params->passphrase);
...
void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	eloop_register_timeout(25 + res->peer_config_timeout / 100,
				  ...
				  int *force_freq, int persistent_group)
{
	...
	if (!persistent_group) {
		wpa_printf(MSG_INFO, "P2P: Invitation from " MACSTR
			   " to join an active group", MAC2STR(sa));
		if (!is_zero_ether_addr(wpa_s->p2p_auth_invite) &&
		    (os_memcmp(go_dev_addr, wpa_s->p2p_auth_invite, ETH_ALEN)
		     == 0 ||
		     os_memcmp(sa, wpa_s->p2p_auth_invite, ETH_ALEN) == 0)) {
			wpa_printf(MSG_INFO, "P2P: Accept previously "
	...
	if (grp) {
		wpa_printf(MSG_INFO, "P2P: Accept invitation to already "
	...
	if (s->mode == WPAS_MODE_P2P_GO && !wpas_p2p_create_iface(wpa_s)) {
		*go = 1;
		if (wpa_s->wpa_state >= WPA_AUTHENTICATING) {
			wpa_printf(MSG_INFO, "P2P: The only available "
		...
		*go = 1;
		...
		{
			wpa_printf(MSG_INFO, "P2P: Failed to allocate a new "
...
accept_inv:
	if (wpa_s->current_ssid && wpa_drv_get_bssid(wpa_s, cur_bssid) == 0 &&
	    wpa_s->assoc_freq) {
		wpa_printf(MSG_INFO, "P2P: Trying to force channel to match "
				   ...
				   struct wpa_scan_results *scan_res)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: Target interface " MACSTR " frequency %d Mhz", 
                MAC2STR(wpa_s->pending_join_iface_addr),freq);
	...
	if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT) && 
		((shared_freq = wpa_drv_shared_freq(wpa_s)) > 0) && freq>0 && (shared_freq != freq)) {
		wpa_msg(wpa_s->parent, MSG_INFO,
		       ...
		       int freq)
{
	...
	/* Make sure we are not running find during connection establishment */
	wpa_printf(MSG_INFO, "P2P: Stop any on-going P2P FIND");
	wpas_p2p_stop_find(wpa_s);

	if (freq == 2) {
		wpa_printf(MSG_INFO, "P2P: Request to start GO on 2.4 GHz "
			   "band");
		if (wpa_s->best_24_freq > 0 &&
		    p2p_supported_freq(wpa_s->global->p2p,
				       wpa_s->best_24_freq)) {
			freq = wpa_s->best_24_freq;
			wpa_printf(MSG_INFO, "P2P: Use best 2.4 GHz band "
				   "channel: %d MHz", freq);
		} else {
			os_get_random((u8 *) &r, sizeof(r));
			freq = 2412 + (r % 3) * 25;
			wpa_printf(MSG_INFO, "P2P: Use random 2.4 GHz band "
				   "channel: %d MHz", freq);
		}
	}
	...
	if (freq > 0 && !p2p_supported_freq(wpa_s->global->p2p, freq)) {
		wpa_printf(MSG_INFO, "P2P: The forced channel for GO "
			   "(%u MHz) is not supported for P2P uses",
			   freq);
		return -1;
	}
...
int wpas_p2p_is_client(struct wpa_supplicant *wpa_s)
{
...
static void wpas_p2p_group_idle_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_INFO, "P2P: JASON the confname is %s, wpa_s->conf->p2p_group_idle = %d, wpas_p2p_is_client = %d ", wpa_s->ifname, wpa_s->conf->p2p_group_idle,wpas_p2p_is_client);
	
	if ((wpa_s->conf->p2p_group_idle == 0 && !wpas_p2p_is_client(wpa_s)) || (os_strcmp(wpa_s->ifname, "ap0") == 0)) {
		wpa_printf(MSG_INFO, "P2P: Ignore group idle timeout - "
			   "disabled");
		return;
	}

	wpa_printf(MSG_INFO, "P2P: Group idle timeout reached - terminate %d "
...
static void wpas_p2p_set_group_idle_timeout(struct wpa_supplicant *wpa_s)
{
    ...
    if(!wpas_p2p_is_client(wpa_s))
        timeout = P2P_MAX_OWNER_IDLE;
...
int wpas_p2p_handle_frequency_conflicts(struct wpa_supplicant *wpa_s, int freq)
{
	...
	wpa_printf(MSG_DEBUG, "wpas_p2p_handle_frequency_conflicts interface %s freq %d", wpa_s->ifname, freq);
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {

		wpa_printf(MSG_DEBUG, "interface %s p2p_group_interface %u", iface->ifname, iface->p2p_group_interface);

        if(iface->current_ssid) {
            wpa_printf(MSG_DEBUG, "current_ssid freq %u ssid %s", 
                    iface->current_ssid->frequency,
                    iface->current_ssid->ssid
                    );
        }

		if(/* Eddie patch  (iface->p2p_group_interface) &&*/ 

                (iface->current_ssid) &&
                iface->current_ssid->p2p_group && 
			(iface->current_ssid->frequency != freq)) {

			/*Eddie if (iface->p2p_group_interface == P2P_GROUP_INTERFACE_GO) {*/
            if(!wpas_p2p_is_client(iface)) {
...
int wpas_p2p_sigma_test_mode(struct wpa_supplicant *wpa_s,
	                           int index, int value)
{
...
int wpas_hotspot_block_list_update(struct wpa_supplicant *wpa_s,
