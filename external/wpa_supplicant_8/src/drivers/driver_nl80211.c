...
broken_combination:
	...
	if (tb[NL80211_ATTR_OFFCHANNEL_TX_OK] && 0) {
...
#ifdef CONFIG_WAPI_SUPPORT
static int wpa_driver_nl80211_set_key_ext(void *priv, enum wpa_alg alg,
				       ...
				       const u8 *key, size_t key_len)
{
   ...
   if(seq_len > IW_ENCODE_SEQ_MAX_SIZE * 2){
	...
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
	NLA_PUT(msg, NL80211_ATTR_TESTDATA, params.hdr.buflen, &params);
				      ...
				      const u8 *key, size_t key_len)
{
    ...
    if (alg == WAPI_ALG_SMS4) {
        ret = wpa_driver_nl80211_set_key_ext(priv, alg, addr, key_idx, set_tx,
	...
	size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    if (wpa_ether_send(drv->ctx, addr, ETH_P_EAPOL, data, data_len) /* send via wpa_s->l2 */) {        
        wpa_printf(MSG_DEBUG, "Sending the Hostapd 1x frame by L2 socket\n");       
        return 0;   
    }    
    else {        
        wpa_printf(MSG_INFO, "Sending the Hostapd 1x frame <<not>> by L2 socket\n");    
    }
...
skip_auth_type:
	...
	if (params->pairwise_suite != CIPHER_NONE) {
        ...
        wpa_printf(MSG_DEBUG, "  * CIPHER %d", cipher);
	...
	if (params->group_suite != CIPHER_NONE) {
        ...
        wpa_printf(MSG_DEBUG, "  * GROUP CIPHER %d", cipher);
				       ...
				       enum nl80211_iftype nlmode)
{
	...
	if (ret) {
		...
		//return ret;
		return 0; /* Not use the monitor interface */
	}
...
#endif
	if (offchanok && (drv->capa.flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) && 0)
...
static int wpa_driver_nl80211_shared_freq(void *priv)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: drv phyname %s", drv->phyname);

	dl_list_for_each(driver, &drv->global->interfaces,
			 struct wpa_driver_nl80211_data, list) {

	wpa_printf(MSG_DEBUG, "nl80211: drv %p driver %p", drv, driver);
	wpa_printf(MSG_DEBUG, "nl80211: driver phyname %s", driver->phyname);
	wpa_printf(MSG_DEBUG, "nl80211: driver associated %u assoc_freq %u", driver->associated, driver->assoc_freq);
	wpa_printf(MSG_DEBUG, "nl80211: driver is_ap_interface nlmode %u first_bss.freq %u", driver->nlmode,driver->first_bss.freq);

		if (drv == driver ||
		   /* Eddie  os_strcmp(drv->phyname, driver->phyname) != 0 || */
#ifdef ANDROID_P2P
...
int wpa_driver_nl80211_send_msg(void *priv, const u8 *msg_in, int msg_in_len,
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.set_test_mode = wpa_driver_nl80211_testmode,
#ifdef HOSTAPD
	.hapd_init = i802_init,
...
/*	.set_noa = wpa_driver_set_p2p_noa,
    ...
    .driver_sw_cmd = wpa_driver_nl80211_driver_sw_cmd,
#ifdef CONFIG_WAPI_SUPPORT
    .msg_send = wpa_driver_nl80211_send_msg
#endif
#endif
};
