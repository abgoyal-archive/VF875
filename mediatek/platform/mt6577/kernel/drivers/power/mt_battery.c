...
void check_battery_id(void){
  ...
  {    
   ...
   if(res < 0)
     ...
     {
	if(Enable_BATDRV_LOG >= 1)
        	printk("[adc_driver]: channel4[%d]raw =%d\n",i,rawdata);
     }
   msleep(5);	
...
///////////////////////////////////////////////////////////////////////////////////////////
extern int CallStatusForCharging;
void select_charging_curret(void)
{
   ...
   //added by chenfei
   if(g_temp_CC_value == AC_CHARGER_CURRENT && CallStatusForCharging != 0) //g_temp_CC_value < USB_CHARGER_CURRENT means that the charing current is higher than usb charing current.
	{
	    g_temp_CC_value = USB_CHARGER_CURRENT;
		//printk("CHENFEI Calling Mode : %d\r\n", USB_CHARGER_CURRENT);
	    if (Enable_BATDRV_LOG == 1) {
                xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] Calling Mode : %d\r\n", USB_CHARGER_CURRENT);
            }  
	}
}
...
void BAT_thread(void)
{
    ...
    int BAT_status = 0;
    kal_uint32 tmp32;
	int detect_counter = 0;     /*jrd add for detect usb type correctly*/
    g_switch_to_i2c_polling_mode = 0;
    ...
    if( upmu_is_chr_det() == KAL_TRUE )
    {
		...
		while( (detect_counter < 6) && (CHR_Type_num == NONSTANDARD_CHARGER) ){
			detect_counter++;
			CHR_Type_num = mt_charger_type_detection();     /*this function may consume 100ms*/
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BAT_thread] poll chgtype CHR_Type_num=%d\r\n", CHR_Type_num);
		}
		detect_counter = 0;
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case Set_Charger_Current: /* For Factory Mode*/
            ...
            break;
            	case TURN_OFF_CHARGING_FOR_FACTORY:
            	    pchr_turn_off_charging();		
            	    printk("turn_off_charging for factory\n");
            	    break;
