...
//// Extern Functions
///////////////////////////////////////////////////////////////////////////////////////////
#define BATT_ID_CHECK_SUPPORT 0

#define VOL_TOP_OFF 4080

#if BATT_ID_CHECK_SUPPORT
#define AUXADC_BATT_ID_CHANNEL 4
#define CHG_MIN_VALID_BATT_ID  600
#define CHG_MAX_VALID_BATT_ID  900
...
#define SHOW_FULL_TIME                      1*45*60	/*jrd add for set bat_full flag when CV time out*/
/*****************************************************************************
 *  BATTERY Protection
 ****************************************************************************/
#define FEATURE_RECOVERY_FROM_HIGH_TEMPERATURE                /*jrd add for recovery charging from  high temperature*/

#define charger_OVER_VOL                    1	/*bit0 set.*/
#define BATTERY_OVER_TEMP		    4	/*bit1 set. jrd add for recovery charging from high temperature */
#define BATTERY_LOW_TEMP		    3	/*bit2 set.*/

#define BATTERY_OVER_RECOVERY_CHARGING_TEMP 43  /*43 -- > 30 for testing*/
#define BATTERY_LOW_RECOVERY_CHARGING_TEMP 3
...
//// Global Variable
/////////////////////////////////////////////////////////////////////
#define FAN5405_CHECK_FAULT_STATUS     1
#if FAN5405_CHECK_FAULT_STATUS
static enum fan5405_charger_status fan_charger_status = CHARGER_INVALID;
static uint8_t chg_on_off_counter = 0;
#endif
...
void fan5405_set_ac_current(void)
{
    ...
    //4). 0x04h->0x19h
    //5). 0x05h->0x04h
    fan5405_config_interface_liao(0x06,0x7a); //set ISAFE :1250mA, 4.2V
    fan5405_config_interface_liao(0x01,0xC8);
    fan5405_config_interface_liao(0x02,0x8A); //set Vcc 4.18V
    fan5405_config_interface_liao(0x04,0x29); //set IOCHARGE 750mA
    fan5405_config_interface_liao(0x05,0x04);
    #endif
}
...
void select_charging_curret_fan5405()
{
    ...
    {        
        ...
        fan5405_config_interface_liao(0x01,0x48);/*Iinlim = 01(500mA), Vlowv = 00(3.4V),*/
		if (Enable_BATDRV_LOG == 1) {
...
void ChargerHwInit_fan5405(void)
{
	...
	if(temp_init_flag==0)
	{
	    fan5405_config_interface_liao(0x06,0x7a);
	    fan5405_config_interface_liao(0x00,0xC0);
	    fan5405_config_interface_liao(0x01,0xb1);
	    fan5405_config_interface_liao(0x02,0x8a);
	    fan5405_config_interface_liao(0x05,0x04);
...
int gpio_number   = GPIO202;		/*GPIO_CHG_DIS = DPIDE()gpio30 on griffinplus , diablo is gpio202*/
...
void pchr_turn_off_charging_fan5405 (void)
{
    ...
    fan5405_config_interface_liao(0x01,0xbc);       /*Iinlim = 800mA, Vlowv = 3.7V, enable TE, CE:charger disable, HZ:0, opa:charge*/       
#if FAN5405_CHECK_FAULT_STATUS 
    fan_charger_status = CHARGER_STOPED;
    chg_on_off_counter = 0; 
#endif
}
...
void pchr_turn_on_charging_fan5405 (void)
{
    ...
    if ( (BMT_status.bat_charging_state == CHR_ERROR) || (BMT_status.charger_protect_status != 0) ) 
    {
        //if (Enable_BATDRV_LOG == 1) {
...
#if FAN5405_CHECK_FAULT_STATUS	
            if (Enable_BATDRV_LOG == 1) {
                printf("[BATTERY] charger enable, enter_counter:%d, f_chg_st:%d!\r\n", chg_on_off_counter, (int)fan_charger_status);
                }    
           if((chg_on_off_counter++ > 3) && (fan5405_get_chip_status() == 0x01))       /*charge in progress*/
                {
                chg_on_off_counter = 4;
                fan_charger_status = CHARGER_IN_PROGRESS_STATUS;
                }
           else if(fan5405_get_chip_status() == 0x02)
               fan_charger_status = CHARGER_FINISHED_STATUS;           
#endif
        }
    }    	
}
...
int BAT_CheckBatteryStatus_fan5405(void)
{
    ...
    if (batteryIndex >= BATTERY_AVERAGE_SIZE)
        batteryIndex = 0;
    /**************** Averaging : END ****************/
#if 0	/*disable this full condition, for displaying battery full level picture correctly*/
    if( BMT_status.SOC == 100 ) {
        BMT_status.bat_full = KAL_TRUE;   
    }
#else
    if( (BMT_status.SOC == 100)&&(BMT_status.bat_full != KAL_TRUE) )
    {
    	bat_volt_check_point = 98;
       BMT_status.SOC = 98;
    }
#endif    
    /**************** For UBOOT : Start ****************/
...
/*** jrd add for led display full when battery level full - */
        if( BMT_status.bat_full == KAL_TRUE )	/*led off now*/
        {
                mt_set_gpio_mode(GPIO67, GPIO_MODE_00);	//PWM1_IND(PWM1)--gpio67
...
#ifdef FEATURE_RECOVERY_FROM_HIGH_TEMPERATURE               
       BMT_status.charger_protect_status = BATTERY_LOW_TEMP;   
#endif

       BMT_status.bat_charging_state = CHR_ERROR;
#if FAN5405_CHECK_FAULT_STATUS
       fan_charger_status = CHARGER_FAULT_NOW;
#endif
        return PMU_STATUS_FAIL;       
    }
	#endif		    	
...
/*jrd add for recovery charging from high temperature */
#ifdef FEATURE_RECOVERY_FROM_HIGH_TEMPERATURE
	if( (BMT_status.charger_protect_status == BATTERY_LOW_TEMP)||(BMT_status.charger_protect_status == BATTERY_OVER_TEMP) ){
		if( (BATTERY_LOW_RECOVERY_CHARGING_TEMP <= BMT_status.temperature )&&( BMT_status.temperature <= BATTERY_OVER_RECOVERY_CHARGING_TEMP ) ){
			BMT_status.charger_protect_status = 0;
			printf("[BATTERY] recovery charging\n" );
		}
		else {
			printf("[BATTERY] recovering though cannot charge \n" );
			return PMU_STATUS_FAIL;
		}
	}
#endif
    ...
    if( (BMT_status.charger_protect_status == BATTERY_OVER_TEMP) || (BMT_status.charger_protect_status == BATTERY_LOW_TEMP) )
               error_reason = 1;
    ...
    if(error_reason == 1 && BMT_status.charger_protect_status == 0)	/**/
    {
            error_reason = 0;
            BMT_status.bat_charging_state = CHR_PRE;
            printf("recovery from high temperature!\n");
#if FAN5405_CHECK_FAULT_STATUS
       if(fan_charger_status != CHARGER_IN_PROGRESS_STATUS)
#endif
...
/*jrd add end*/
    ...
    if (upmu_is_chr_det() == KAL_TRUE)
    {
        if ((BMT_status.bat_vol < RECHARGING_VOLTAGE) && (BMT_status.bat_full) && (g_HW_Charging_Done == 1))	
        {
            ...
            //if(fan_charger_status != CHARGER_IN_PROGRESS_STATUS)
#endif
	//add by carl.li@jrdcom.com for pr375852 @2012-12-19
	    fan5405_config_interface_liao(0x06,0x7a);
	    fan5405_config_interface_liao(0x00,0xC0);
	    fan5405_config_interface_liao(0x01,0xb1);
	    fan5405_config_interface_liao(0x02,0x8a);
...
void check_battery_id(void){
  ...
  {    
   ...
   #else
       tmo2 = get_timer(0);
       while(get_timer(tmo2) <= 200 /* ms */);
  #endif

   bat_id_vol += (rawdata * 2500 / 4096);
  }
  ...
  //printf("bat_id_vol:%d!\n", bat_id_vol);    
  if((bat_id_vol < CHG_MIN_VALID_BATT_ID) || (bat_id_vol > CHG_MAX_VALID_BATT_ID)) /*battery id error*/
       {
       printf("battery id error %d!\n", bat_id_vol);
       pchr_turn_off_charging_fan5405();
//	leds_battery_charger_plugout();		/*jrd add for close led when battery id error*/
        #ifndef NO_POWER_OFF
        mt6577_power_off();
        #endif
        while(1);      
...
void check_battery_exist(void)
{
    ...
    if( baton_count >= 3)
    {
            ...
            else
            {
                ...
                pchr_turn_off_charging_fan5405();
		//leds_battery_charger_plugout();		/*jrd close leds when battery not exist FIXME*/
        #ifndef NO_POWER_OFF
                mt6577_power_off();       
	#endif
		while(1); 
...
void BAT_thread_fan5405(void)
{
    ...
    fan5405_config_interface_liao(0x00,0xC0);	/*reset t32 and enable STAT pin low when charging*/
   ...
   // if ( upmu_is_chr_det() == KAL_FALSE )
    ...
    {
        //bmt_charger_ov_check();	/*jrd delete -see below "jrd add"*/
...
#if FAN5405_CHECK_FAULT_STATUS
        fan_charger_status = CHARGER_TYPE_CHANGED;
#endif
    ...
    else
    {
       ...
       if( BMT_status.bat_vol >= VOL_TOP_OFF )
       {
              BMT_status.bat_charging_state = CHR_TOP_OFF;
       ...
       if( ((fan5405_status == 0x2) && (fan_charger_status == CHARGER_IN_PROGRESS_STATUS)) || 
		((BMT_status.bat_charging_state == CHR_TOP_OFF)&&(BMT_status.TOPOFF_charging_time > SHOW_FULL_TIME)))
    	{
    		BMT_status.bat_charging_state = CHR_BATFULL;
    		BMT_status.bat_full = KAL_TRUE;
    		BMT_status.total_charging_time = 0;
...
#if FAN5405_CHECK_FAULT_STATUS
static uint8_t check_fan5405_fault_status(uint8_t fault_bit){

       uint8_t ret = 1;

        if(fault_bit)
                printf("jrd %s: enter fault_bit:0x%x!\n\r",__func__, fault_bit);

        switch(fault_bit){
        case CHARGE_NORMAL_STATUS:
                        BMT_status.bat_charging_state = CHR_PRE;        /*recovery bat status*/
                        BMT_status.total_charging_time = 0;     /**/
                        BMT_status.PRE_charging_time = 0;       /**/
                        BMT_status.CC_charging_time = 0;        /**/
                        BMT_status.TOPOFF_charging_time = 0;    /**/
                        ...
                        pchr_turn_on_charging_fan5405();
                       fan_charger_status = CHARGER_RECOVERY_FROM_FAULT;
                       ret = 0;
                break;
        case CHARGE_VBUS_OVP_STATUS:
        case CHARGE_SLEEP_MODE:
        case CHAREG_POOR_INPUT_SOURCE:
               pchr_turn_off_charging_fan5405();
               fan_charger_status = CHARGER_FAULT_NOW;
                break;
        case CHAREG_BATTERY_OVP:
               g_Battery_Fail = KAL_TRUE;
               pchr_turn_off_charging_fan5405();
               fan_charger_status = CHARGER_FAULT_NOW;
                break;
        case CHARGE_THERMAL_SHUTDOWN:
               ...
               fan_charger_status = CHARGER_FAULT_NOW;
                break;
        case CHARGE_TIMER_FAULT:
                /*T15min or t32sec expires, re-start*/
               fan5405_config_interface_liao(0x00, 0xC0);
               fan_charger_status = CHARGER_RECOVERY_FROM_FAULT;
                pchr_turn_on_charging_fan5405();
               ret = 0;
                break;
        case CHARGE_NO_BATTERY:         /*FIXME battery id check ieee1725*/
        case CHARGE_INVALID_STATUS:
                break;
        default:
                break;
        }
...
{
#if FAN5405_CHECK_FAULT_STATUS
     unsigned int fan_status, fan_fault_status, check_ret;
...
#endif
     {
	     ...
	     { 
#if FAN5405_CHECK_FAULT_STATUS 
               fan_fault_status = fan5405_get_fault_reason();  /**/
#endif
#if 0	/*griffinplus add these sentence FIXME*/
...
#if FAN5405_CHECK_FAULT_STATUS
                else if(upmu_chr_get_vcdt_hv_det() == 0)        /*charger < 6.5V*/
                {
                    charger_ov_boot_display = 0;
                   mt65xx_backlight_off();         
                    break;  
                }
#endif  
...
{
    #define BATTERY_BAR 10	/*jrd change from 25 --> 10*/
    ...
    {	
        ...
        {	
            ...
            mt65xx_disp_show_battery_capacity(prog); // jrd change for mask battery level "xx%" display Charging Animation
#else	
            mt65xx_disp_show_battery_capacity(prog);
#endif
...
//        user_view_flag = KAL_TRUE;	/*jrd add for indicator led displying battery full flag correctly*/
    } else {
...
#if FAN5405_CHECK_FAULT_STATUS
       if(fan5405_get_chip_status() == 0x00)
               fan_charger_status = CHARGER_READY_STATUS;
#endif 
        ...
        while (1) 
        {
			...
			//mt65xx_backlight_off();
                    ...
                    {
                        mt65xx_disp_wait_idle(); //add by carl solve abnormal animation when quit poweroffcharging
                        mt65xx_disp_show_boot_logo();
	            ...
	            if( BMT_status.bat_full == KAL_TRUE ) //LED OFF
        	    {
            ...
            if(print_msg==FALSE)
            {	
                //#ifdef ANIMATION_NEW 		/*jrd delete FIXME???*/
                BAT_CheckBatteryStatus_fan5405();  
                //#endif			/*jrd delete FIXME ???*/
			...
			bl_switch_timer = 0;
			g_bl_on = true;
			printf("[BATTERY] turn on backlight\r\n");
             }
    ...
    else	/*boot when no charger on*/
    {
        //bmt_charger_ov_check();	/*not necessory to check charger ov or fan fault status*/	
