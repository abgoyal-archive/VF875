...
#define BATTERY_OVER_TEMP 4
#define BATTERY_LOW_TEMP 3
#define BATTERY_OVER_RECOVERY_CHARGING_TEMP 43
...
void pchr_turn_on_charging (void)
{
    ...
    else
    {
        ...
        {			
			...
			upmu_chr_vbat_cv_vth(0x1F);				// modified by chenfei,0x1F -- 4.1875V
...
{ 
    ...
    //modified by chenfei,for calibration battery voltage
    if(BMT_status.ICharging >= 120)
    {
	if(g_bl_on)
	BMT_status.bat_vol = (UINT32)(BMT_status.ADC_BAT_SENSE - ((BMT_status.ICharging-120)*130)/1000); 
	else
	BMT_status.bat_vol = (UINT32)(BMT_status.ADC_BAT_SENSE - (BMT_status.ICharging*130)/1000);
    }
    else
    {
    	BMT_status.bat_vol = BMT_status.ADC_BAT_SENSE;
    }
    //printf("CHENFEI : VBAT RAW Data : %d, ICharging RAW Data : %d BL Status : %d \n",BMT_status.ADC_BAT_SENSE,BMT_status.ICharging,g_bl_on);
...
int BAT_CheckBatteryStatus(void)
{
    ...
    if (!batteryBufferFirst)
    {
	    ...
	    batteryTemperatureBuffer[i] = BMT_status.temperature;
        }
	...
	batteryTemperatureSum = BMT_status.temperature * BATTERY_AVERAGE_SIZE;
    }
    ...
    batteryTemperatureSum -=batteryTemperatureBuffer[batteryIndex];
    batteryTemperatureSum += BMT_status.temperature;
    batteryTemperatureBuffer[batteryIndex] = BMT_status.temperature;
    ...
    BMT_status.temperature = batteryTemperatureSum / BATTERY_AVERAGE_SIZE;
...
#if 1        
        /* UBOOT charging LED */
        if ( (bat_volt_check_point >= 100)  || (user_view_flag == KAL_TRUE) ) {  //modify 90 to 100 for led off checkpoint
    ...
    {
	if(BMT_status.temperature > BATTERY_OVER_RECOVERY_CHARGING_TEMP)
	{
	...
	if(BMT_status.temperature < BATTERY_LOW_RECOVERY_CHARGING_TEMP)
	{
    ...
    #endif 
    //end added by chenfei

	if( BATTERY_LOW_RECOVERY_CHARGING_TEMP <= BMT_status.temperature <= BATTERY_OVER_RECOVERY_CHARGING_TEMP )  
...
void check_point_sync_leds(void)
{
	...
	if(battery_level >= 100)    //Full ARGB //modify by chenfei, 90 to 100 for led off checkpoint
	{
		leds_battery_full_charging();
	}
...
{    
	...
	BAT_GetVoltage_notbat();
   //modified by chenfei,for improving the leds and charging logo style,the battery full is the only condition for leds off when charging.
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))	
    {
                ...
                if(back_to_charging_animation_flag == false)
		{
                ...
                {
                    mt65xx_disp_wait_idle(); //add by yuan-li@tcl.com for PR428713 @2013.02.25
                    mt65xx_disp_show_boot_logo();
