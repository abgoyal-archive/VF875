...
//add by sunjiaojiao for beetlelite jb start
#define INDICATOR_GPIO_NUM     GPIO110
//add by sunjiaojiao for beetlelite jb end    
#define NLED_OFF 0
#define NLED_ON 1
#define NLED_BLINK 2
...
static int backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	...
	if(level>0 && level <= 32)
	{
		...
		//added by xiewei
		//pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA0 =  0x0000FFFF;
		//pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA1 = 0xFFFFFFFF & (~((1 << level)-1));
		//pwm_set_spec_config(&pwm_setting);
	...
	}else if(level>32 && level <=64)
	{
		...
		pwm_set_spec_config(&pwm_setting);	
		//added by xiewei
		//pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA0 = 0xFFFFFFFF & (~((1 << level)-1));
		//pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA1 = 0;
		//pwm_set_spec_config(&pwm_setting);	
...
static int mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	switch (cust->mode) {
		case MT65XX_LED_MODE_PWM:
			...
			{
				mt_set_gpio_mode(GPIO68, GPIO_MODE_01);//added by xiewei  for PWM3 BL
				bl_brightness = level;
				if(level == 0)
				{
			...
			}else
			{
				if(level == 0)
				{
					if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ZERO);//modify by sunjiaojiao for beetlelite jb
						}
				...
				}else
				{
					if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ONE);//modify by sunjiaojiao for beetlelite jb
						}
...
static ssize_t store_notify_led(struct device *dev, struct device_attribute *attr ,const char *buf,size_t size)
{
	...
	struct cust_mt65xx_led *cust_led_list =get_cust_led_list();
	struct nled_setting led_tmp_seting, led_tmp_seting_event;
	led_tmp_seting.nled_mode = NLED_BLINK;
	led_tmp_seting.blink_off_time = delayon;
	led_tmp_seting.blink_on_time = delayoff;

	led_tmp_seting_event.nled_mode = NLED_BLINK;
	led_tmp_seting_event.blink_off_time = 500;
	led_tmp_seting_event.blink_on_time = 4500;
	if(buf != NULL && size != 0)
		{
			...
			if(notify_led == 2)
				{
					mt_set_gpio_out(INDICATOR_GPIO_NUM,GPIO_OUT_ONE);//modify by sunjiaojiao for beetlelite jb
					notify_led_enable = 2;
					led_set_pwm(PWM1, &led_tmp_seting_event);
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if(strcmp(g_leds_data[i]->cdev.name,"red")==0)
		{
			...
			if(rc)
			{
				LEDS_DEBUG("[notify_led] device create file duty fail!\n");	
			}
			...
			mt_set_gpio_mode(INDICATOR_GPIO_NUM, GPIO_MODE_00);//modify by sunjiaojiao for beetlelite jb
			mt_set_gpio_dir(INDICATOR_GPIO_NUM, GPIO_DIR_OUT);//modify by sunjiaojiao for beetlelite jb
			mt_set_gpio_out(INDICATOR_GPIO_NUM, GPIO_OUT_ZERO);//modify by sunjiaojiao for beetlelite jb
			mt65xx_led_set_cust(&cust_led_list[0], 0);
		}
		if (ret)
